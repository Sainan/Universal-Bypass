...
ensureDomLoaded(()=>{
	...
	domainBypass("5mod-file.ru",()=>ensureDomLoaded(()=>ifElement("a.download",a=>safelyNavigate(a.href))))
	domainBypass("kazanclilink.com",()=>ensureDomLoaded(()=>ifElement("a#baglantigit",a=>safelyNavigate(a.href))))
	domainBypass("moddingunited.xyz", () => ensureDomLoaded(() => {
	    let originalLink =window.location.search.substring(1);
	    let newLink =reverseString(originalLink); // Uses reverseString() funciton from the site

	    safelyNavigate(newLink)
	}));
